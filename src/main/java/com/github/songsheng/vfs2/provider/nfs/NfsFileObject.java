...
public class NfsFileObject extends AbstractFileObject<NfsFileSystem> {
	...
	protected NfsFileObject(final AbstractFileName name, final NfsFileSystem fileSystem) throws FileSystemException {
	...
	private XFile createNfsFile(final FileName fileName) throws MalformedURLException, FileSystemException {
		...
		try {
			// no auth for nfs
			/*
			 * authData = UserAuthenticatorUtils.authenticate( getFileSystem().getFileSystemOptions(), NfsFileProvider.AUTHENTICATOR_TYPES);
			 */

			file = new XFile(path);

			/*
			 * if (file.isDirectory() && !file.toString().endsWith("/")) { file = new XFile(path + "/"); }
			 */
			return file;
		} finally {
