...
public class NfsFileNameParser extends GenericURLFileNameParser {
	...
	public FileName parseUri(final VfsComponentContext context, final FileName base, final String filename) throws FileSystemException {
		...
		final Authority auth = extractToPath(null, filename, name);
		...
		return new NfsFileName(auth.getScheme(), auth.getHostName(), auth.getPort(), username, auth.getPassword(), path, fileType);
	}
